    ...
    return (
        <aside
            className={`
                ...
                flex flex-col transition-all duration-300 ease-in-out dark:bg-[#262626] bg-white dark:border-[#303030]
            `}
...
export function SideBarHeader({ children }) {
    return (
        <header className="px-6 py-5 flex justify-between transition-all duration-300 hover:bg-gray-100 dark:hover:bg-gray-700 dark:text-white">
            {children}
        </header>
    );
}
...
export function SideBarNavLink({ children, to }) {
    return (
        <Link
            className="flex items-center mb-2.5 no-underline text-black dark:text-gray-200 text-sm px-3 py-2 gap-3 rounded-lg transition-all duration-300 hover:bg-gray-200 dark:hover:bg-gray-700"
            to={to}
        >
            {children}
        </Link>
